...
public class SDDataBaseHelper extends SQLiteOpenHelper {
    private String Tag = SDDataBaseHelper.class.getSimpleName();
    // 用户数据库文件的版本
    private static final int DB_VERSION = 1;
    // 数据库文件目标存放路径为系统默认位置，cn.arthur.examples 是你的包名
    private static String DB_PATH = "/data/data/ch4checkpro.com.cn/databases/";
    /*
     ...
     */
    public SDDataBaseHelper(Context context, String name, CursorFactory factory, int version) {
        // 必须通过super调用父类当中的构造函数
        super(context, name, null, version);
        this.myContext = context;
        DB_NAME = AppUtil.packageName(context);
        ASSETS_NAME = DB_NAME;
    }

    public SDDataBaseHelper(Context context, String name, int version) {
        this(context, name, null, version);
    }

    public SDDataBaseHelper(Context context, String name) {
    ...
    public void createDataBase() throws IOException {
        ...
        if (dbExist) {
            ...
            try {
                ...
                copyDataBase();
                Log.i(Tag, "Success copy database from assets");
            } catch (IOException e) {
